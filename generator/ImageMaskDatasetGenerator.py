...
class ImageMaskDatasetGenerator:
  ...
  def colorize_mask(self, mask, color):
    h, w = mask.shape[:2]
    (r, g, b) = color
  ...
  def generate(self, masks_dir, output_images_dir, output_masks_dir, category):
    category_color_map = {"melanoma":(255,0,0), "notmelanoma":(0,255,0), }
    subdirs =  category_color_map.keys()
    subdirs = os.listdir(masks_dir)
    index = 10000
    color  = category_color_map[category]
    for subdir in subdirs:
      index += 1000
      subdir_fullath = os.path.join(masks_dir, subdir)
    
      mask_files = sorted(glob.glob(subdir_fullath + "/*_contour.png"))
      num = len(mask_files)
      ...
      for i in range(num):
        ...
        image_file = mask_file.replace("_contour.png", "_orig.jpg")
        image = cv2.imread(image_file)
        mask  = cv2.imread(mask_file)
        mask  = cv2.cvtColor(mask, cv2.COLOR_RGB2GRAY)
        mask  = self.colorize_mask(mask, color)     
        out_filename = str(index) + "_" + str(i+1) + ".png"
        out_imagefilepath = os.path.join(output_images_dir, out_filename )   
        out_maskfilepath = os.path.join(output_masks_dir, out_filename )   

        cv2.imwrite(out_maskfilepath, mask)
        print("Savd {}".format(out_maskfilepath))
        cv2.imwrite(out_imagefilepath, image)
        print("Savd {}".format(out_imagefilepath))

        if self.augmentation:
          self.augment(mask, out_filename, output_masks_dir, border=(0, 0, 0),     mask=True)
          self.augment(image, out_filename, output_images_dir, border=(0, 0, 0),   mask=False)
...
if __name__ == "__main__":
  try:
    masks_dir    = "./skin-cancer/melanoma/"   

    output_dir   = "./Augmented-Skin-Cancer-Melanoma"
    ...
    generator = ImageMaskDatasetGenerator(augmentation=True)
    generator.generate(
                        masks_dir,
    ...
    masks_dir    = "./skin-cancer/notmelanoma/"   
    generator.generate(
                        masks_dir,
                        output_images_dir, 
                        output_masks_dir,
                        category="notmelanoma")
  except:
    traceback.print_exc()